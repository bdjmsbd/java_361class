...
public class ScheduleManager implements Program {
	...
	/**
	 * 주어진 리스트를 번호. 날짜 일정 : 상세 순으로 출력하고 출력된 결과가 있는지 없는지 알려주는 메소드
	 * 
	...
	/**
	 * 날짜를 입력하여 입력한 날짜에 맞는 일정들을 출력하고, 출력된 일정을 선택하여 객체로 반환하는 메소드(재사용을 위해 만듬)
	 * 
	 * @param 수정 또는 삭제
	 * @return 선택한 일정 객체
	 */
	private Schedule selectSchedule(String type, List<Schedule> sList) {
		...
		List<Schedule> searchList = scheduleFilter(s -> s.getDateStr().substring(0, 10).equals(date), sList);
