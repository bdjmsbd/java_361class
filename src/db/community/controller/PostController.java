...
public class PostController {
	...
	public boolean updatePost(int po_num) {
		...
		PostVO newPost = new PostVO(po_num, new_title, new_content);

		return postService.postUpdate(newPost);
	...
	public void insertComment(PostVO post, String id) {
		...
		String content = scan.nextLine();
		CommentVO commment = new CommentVO(content, post.getPo_num(), id);

		if (postService.insertComment(commment)) {
	...
	public void printCommentList(PostVO post) {
		...
		// 있으면 댓글 리스트에서 하나씩 꺼내 출력.
		List<CommentVO> list = null;
		try {
			list = postService.getCommentList(post.getPo_num());
		} catch (Exception e) {
			System.out.println("등록되지 않은 게시글이거나 삭제된 게시글 입니다.");
			return;
		}
		...
		for (CommentVO co : list) {
			if (co.getCm_ori_num() != co.getCm_num()) {
				System.out.print(" -");
			}
