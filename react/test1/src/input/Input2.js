...
function Input2(){
	...
	function change(e){
		...
		*/
	let change = e=>setTime(e.target.value);
	let btnClick = ()=>{
		setFlag(!flag);
		setTime(0);
	}
	...
	return (
		<div>
			<input type="number" disabled={!flag} value={time==0?"":flag?time:Math.floor(time/60)} onChange={change}/>
			<label>분</label>
			<button onClick={btnClick}>전환</button>
			<input type="number" disabled={flag} value={time==0?"":flag?time*60:time} onChange={change} step="10"/>
			<label>초</label>
		</div>
	)
}
