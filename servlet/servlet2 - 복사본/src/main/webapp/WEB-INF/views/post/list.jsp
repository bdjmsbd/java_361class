...
<body>
	...
	<div class="container" style="min-height: calc(100vh - 240px)">
		<h1 class="mt-3 mb-3">${co.co_name } 커뮤니티</h1>
		<form action="">
			...
			<tbody>
				<c:forEach items="${list }" var="post">
					<tr>
						<td>${post.po_num }</td>
						<td><a class="nav-link"
							href="<c:url value="/post/detail?po_num=${post.po_num}"/>">
								${post.po_title }</a></td>

						<td><c:url value="/post/list" var="url">
								<c:param name="type" value="writer" />
								<c:param name="search" value="${post.po_me_id }" />
								<c:param name="co_num" value="${pm.cri.co_num }" />
							</c:url> <a href="${url}">${post.po_me_id}</a></td>
				...
				<c:if test="${list.size() == 0 }">
					<tr>
						<th colspan="5" class="text-center">일치하는 게시글이 없습니다.</th>
					</tr>
				</c:if>
			</tbody>
		</table>
