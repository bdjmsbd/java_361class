...
<body>
	...
	<div class="container">
		...
		<form class="input-group mb-3" action="<c:url value="/post/list"/>">
			<div class="input-group-prepend">
				<select class="form-control" name="type">
					...
					<option value="id"
						<c:if test="${pm.cri.type == 'id' }">selected</c:if>>작성자</option>
				</select>
			</div>
			...
			<tbody>
				<c:forEach items="${list}" var="post" varStatus="vs">
					<tr>
						...
						<!--<td>${post.po_num}</td>-->
						<td><a href="<c:url value="detail?num=${post.po_num}"/>">${post.po_title}</a>
						</td>
						<td>${post.po_me_id}</td>
						...
						<td><c:choose>
								...
								<c:otherwise>${post.po_up}/<c:if
										test="${post.po_down ne 0}">-</c:if>${post.po_down}</c:otherwise>
							</c:choose></td>
		...
		<a href="<c:url value="/post/insert?co_num=${pm.cri.co_num}"/>"
			class="btn btn-outline-danger">게시글 등록</a>
	</div>
</body>
</html>