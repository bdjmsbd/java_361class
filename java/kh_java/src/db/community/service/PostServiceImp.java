...
public class PostServiceImp implements PostService {
	...
	@Override
	public boolean postUpdate(PostVO newPost) {
		// TODO Auto-generated method stub
		return postDao.updatePost(newPost);
	}

	@Override
	public boolean insertComment(CommentVO commment) {
		// null체크해서 문제 있으면 false를 리턴
		if (commment == null) {
			return false;
		}
		// 댓글 내용 null체크, 빈문자열 체크해서 문제 있으면 false를 리턴
		if (!checkString(commment.getCm_content())) {
			return false;
		}
		// 다오에게 게시글 VO를 주면서 게시글을 등록하라고 요청한 후 성공 여부를 반환
		try {
			return postDao.insertComment(commment);
		} catch (Exception e) {
			return false;
		}
	}
	...
	@Override
	public boolean updatePostView(int poNum) {

		return postDao.updatePostView(poNum);
